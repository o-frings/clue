...
const TAXONOMY = [
  ...
  { id:'natural', label:'Natural sciences', icon:'🔬', disciplines:[
    { id:'physics', label:'Physics',          icon:'⚛️', fields:['phys-mech','phys-em','phys-thermo','phys-waves','phys-rel','phys-quantum','phys-particle','phys-condensed','phys-astro'] },
    { id:'biology', label:'Biology',          icon:'🧬', fields:['biocell','biogen','bioevo','biophysio','biomicro','biodev','bioneuro'] },
  ...
  { id:'social', label:'Social sciences', icon:'👥', disciplines:[
    ...
    { id:'polsci',  label:'Political science', icon:'🏛️', fields:['polth','polidea','polcomp','polelec','polmeth','ir'] },
    ...
    { id:'geog',    label:'Geography',         icon:'🌍', fields:['geopol','geohuman','georegion'] },
  ]},
  ...
  { id:'applied', label:'Applied & professional', icon:'💼', disciplines:[
    ...
    { id:'law',     label:'Law',               icon:'⚖️', fields:['lawth','lawpub','lawpriv'] },
  ]},
...
function renderMotionHome(m){
  ...
  const sparBtns = m.spar ? (function(){ const p=m.spar[side].persona;
    ...
    return sideSeg+
      ...
      '<div class="dbhubmain"><div class="dbhubt">Hard mode<span class="dbhardtag">no hints</span></div>'+
      '<div class="dbhubd">'+esc(p.name)+' takes you seriously: moves unlabelled, no multiple-choice — recall the fact or concede.</div></div>'+
      '<span class="mpchev">›</span></button>'; })() : '';
  $("dbBody").innerHTML=
...
function startSpar(m, tier, side){ side = (side && m.spar[side]) ? side : (m.spar.for?'for':Object.keys(m.spar)[0]);
  // a persisted, ever-incrementing counter rotates which recall variant each round
  // demands, so a rematch isn't identical. `vidx` is fixed for the life of this bout.
  const vidx = (settings.sparPlays=(settings.sparPlays||0)+1); persistAll();
  spar={ side, tier:tier||'spar', vidx, round:0, tried:[], rc:[], hits:0, done:false, failed:false }; dbStage='spar'; renderDebate(); }
...
// the recall demanded this round: rotates through the right move's variant pool
// (`recalls`), falling back to the single legacy `recall`. Stable within a bout.
function roundRecall(rd, r){ const rm=rightMove(rd); const pool = (rm.recalls && rm.recalls.length) ? rm.recalls : (rm.recall ? [rm.recall] : []);
  return pool.length ? pool[((spar.vidx||0)+r) % pool.length] : null; }
...
function renderSpar(m){
  ...
  for(let r=0; r<=spar.round && r<rounds.length; r++){
    ...
    if(pickedRight(r,rd)){ const rec=roundRecall(rd,r); if(rec){ const rc=rcOf(r);
      html+=bubble('opp', esc(rec.demand), 'attack');
      const nag = spar.tier==='pro' ? 'Not quite. Try again — there are no hints in here.' : 'Not quite. Again — or reveal the options.';
      rc.tries.forEach(t=>{ html+=bubble('me', esc(t.resp), t.ok?'land':'wrong-pick');
        html+=bubble('opp', esc(t.ok? rec.good : nag), t.ok?'concede':'jab'); });
      if(rc.revealed && !rc.done && rec.miss) html+=bubble('opp', esc(rec.miss), 'jab');
    } }
  }
  html+='</div>';
  // controls
  if(spar.done && spar.failed){
    html+='<div class="sparverdict failed">'+
      '<div class="svk">✋ The floor slipped away</div>'+
      '<p>'+esc(S.lose)+'</p>'+
      '<div class="svhits">The fact you couldn’t cite: <b>'+esc(spar.failedAnswer)+'</b></div>'+
    ...
    const hard = spar.tier==='pro';
    const used=[]; rounds.forEach(rd=> rd.moves.forEach(mv=>{ if(mv.right && mv.tactic && used.indexOf(mv.tactic)<0) used.push(mv.tactic); }));
    const cards=[]; rounds.forEach((rd,ri)=>{ const rec=roundRecall(rd,ri); if(rec && rec.card && byId[rec.card] && cards.indexOf(rec.card)<0) cards.push(rec.card); });
    html+='<div class="sparverdict '+(flawless?'flawless':'')+'">'+
      '<div class="svk">'+(flawless?'🏆 Flawless — not a hit landed':'✅ You held the floor')+(hard?' <span class="sparhard">🎓 Hard mode</span>':'')+'</div>'+
      '<p>'+esc(S.win)+'</p>'+
    ...
    if(!pickedRight(r,rd)){
      ...
      html+='<div class="sparmovetip">Your move — '+(spar.tier==='pro'?'name the right tactic':'pick the strongest answer')+'</div>';
      html+='<div class="sparmoves">'+order.map(i=>{ const mv=rd.moves[i];
        return '<button class="sparmove" data-i="'+i+'">'+esc(sparLabel(mv.label))+'</button>'; }).join('')+'</div>';
    } else { const rec=roundRecall(rd,r), rc=rcOf(r); if(rec && !rc.done){
      html+='<div class="sparrc">';
      const inputHtml='<div class="quiztype"><input id="rcInput" class="qzin" type="text" inputmode="'+(rec.qz.type==='num'?'decimal':'text')+'" autocomplete="off" autocapitalize="off" spellcheck="false" placeholder="Type a name, number or date"><button class="btn wide sm qzcheck" id="rcCheck">Cite it</button></div>';
      if(spar.tier==='pro'){
        html+='<div class="sparmovetip">Cite the fact — no options, no hints</div>'+inputHtml;
        if((rc.tries||[]).length>=2) html+='<button class="rcreveal" id="rcConcede">Can’t recall it? Concede the bout</button>';
      } else if(!rc.revealed){
        html+='<div class="sparmovetip">Back it up — cite the fact</div>'+inputHtml+
        ...
        html+='<div class="sparmovetip">Pick the right one</div><div class="quizopts">'+
          rec.choices.map((ch,ci)=> triedC.indexOf(ci)>=0?'':'<button class="quizopt rcopt" data-c="'+ci+'">'+esc(ch)+'</button>').join('')+'</div>';
      }
      html+='</div>';
    } }
  }
  ...
  const rcCon=$("rcConcede"); if(rcCon) rcCon.onclick=()=>sparConcede(m);
  const rm=$("sparRematch"); if(rm){ const tier=spar.tier, side=spar.side; rm.onclick=()=>startSpar(m, tier, side); }
...
function sparPick(m, i){
  ...
  if(mv.right){ toast('🎯 Landed — '+(byId[mv.tactic]? byId[mv.tactic].title : 'good move'), true);
    if(!roundRecall(rd, spar.round)) resolveRound(m);   // no fact to cite → round done
  } else { spar.hits++; toast('✋ '+(m.spar[spar.side].persona.name)+' counters'); }
  renderDebate();
}

function sparRecallSubmit(m, resp){ if(!String(resp).trim()) return;
  const rd=m.spar[spar.side].rounds[spar.round], rec=roundRecall(rd,spar.round), rc=rcOf(spar.round);
  const ok=gradeQuizAnswer(rec.qz, resp); rc.tries.push({resp:String(resp).trim(), ok:ok});
...
// hard mode only: give up the recall, ending the bout as a loss but showing the answer to learn
function sparConcede(m){ const rd=m.spar[spar.side].rounds[spar.round], rec=roundRecall(rd,spar.round);
  spar.failed=true; spar.failedAnswer=String(rec.qz.answer); spar.done=true;
  toast('Conceded the bout'); renderDebate(); }
function sparRecallPick(m, ci){
  const rd=m.spar[spar.side].rounds[spar.round], rec=roundRecall(rd,spar.round), rc=rcOf(spar.round);
  const ok = ci===rec.correctChoice; rc.tries.push({resp:rec.choices[ci], ok:ok, choice:ci});
...
function drawChord(){ try{
  ...
  const cx=cssW/2, cy=cssH*0.52, rad=Math.min(cssW,cssH)/2, Ri=rad*0.22, Hmax=rad*0.44, rIn=Ri-3;   // small hole, tall bars, outer margin for icons
...
function renderWeb(){
  ...
  if(cvCh){
    ...
    cvCh.onclick=(ev)=>{ const h=chordHit(cvCh,ev); if(!h) return;
      ...
      const cue=cvCh.parentElement && cvCh.parentElement.querySelector('.ch-cue');
      if(cue){ const fo=_chordFocus && _chordModel && _chordModel.fields.filter(f=>f.id===_chordFocus)[0]; cue.textContent = fo? fo.label : 'tap a field'; } };
...
// v121: History split into period + thematic sub-fields (like Economics). Card progress is keyed
// by card id, so nothing learned is lost; only field-keyed settings are remapped.
function migrateTaxonomyV121(){
  if(settings._taxo121) return;
  const HIST=['histanc','histmed','histearly','histmod','histcontemp','histglobal','histideas','histmethod'];
  if(Array.isArray(settings.focus)){
    const out=[]; settings.focus.forEach(f=>{ (f==='history'?HIST:[f]).forEach(x=>out.push(x)); });
    settings.focus=[...new Set(out)];
  }
  delete settings.degrees;
  settings._taxo121=true;
  persistAll();
}

// v122: Physics split into 9 sub-fields (mechanics, EM, thermo, waves, relativity,
// quantum, particle/nuclear, condensed matter, astrophysics). Progress is card-id keyed.
function migrateTaxonomyV122(){
  if(settings._taxo122) return;
  const PHYS=['phys-mech','phys-em','phys-thermo','phys-waves','phys-rel','phys-quantum','phys-particle','phys-condensed','phys-astro'];
  if(Array.isArray(settings.focus)){
    const out=[]; settings.focus.forEach(f=>{ (f==='physics'?PHYS:[f]).forEach(x=>out.push(x)); });
    settings.focus=[...new Set(out)];
  }
  delete settings.degrees;
  settings._taxo122=true;
  persistAll();
}

// v123: Biology split into 7 sub-fields (molecular/cell, genetics, evolution, physiology,
// microbiology/immunology, developmental, neuroscience). Progress is card-id keyed.
function migrateTaxonomyV123(){
  if(settings._taxo123) return;
  const BIO=['biocell','biogen','bioevo','biophysio','biomicro','biodev','bioneuro'];
  if(Array.isArray(settings.focus)){
    const out=[]; settings.focus.forEach(f=>{ (f==='bio'?BIO:[f]).forEach(x=>out.push(x)); });
    settings.focus=[...new Set(out)];
  }
  delete settings.degrees;
  settings._taxo123=true;
  persistAll();
}
...
// v127: final cleanup — Political science (theory/ideologies/comparative/elections/methods),
// Law (theory/public/private), Geography (geopolitics/human/regional). Progress is card-id keyed.
function migrateTaxonomyV127(){
  if(settings._taxo127) return;
  const map={ polisci:['polth','polidea','polcomp','polelec','polmeth'],
              law:['lawth','lawpub','lawpriv'],
              geo:['geopol','geohuman','georegion'] };
  if(Array.isArray(settings.focus)){
    const out=[]; settings.focus.forEach(f=>{ (map[f]||[f]).forEach(x=>out.push(x)); });
    settings.focus=[...new Set(out)];
  }
  delete settings.degrees;
  settings._taxo127=true;
  persistAll();
}
